...
Trying to R&D for possiblity of integrating Raydium AMM DEX with my own UI in react
...
export async function getMarket(conn: any, marketAddress: string): Promise<any | null> {
  try {
    const expectAmmId = (
...
async function initAmm(
  ...
  if (market.baseMintAddress.toString() === TOKENS.WSOL.mintAddress) {
    const newAccount = Keypair.generate(); //Account(), since Account is deprecated, 
                                            // should change to Keypair.generate()
    transaction.add(
  ...
  if (market.quoteMintAddress.toString() === TOKENS.WSOL.mintAddress) {
    ...
    // replace Account() by Keypair.generate()
    const newAccount =  Keypair.generate();  //new Account() 
