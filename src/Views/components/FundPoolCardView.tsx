...
export const FundPoolCardView : React.FC <FundPoolCardViewProps> = ({address, manager, 
    ...
    return <div className={className}>
        ...
        src={(icon ?? 0) >= 0 ? ICONS[icon ?? 0] : "none"}
        ...
        <span className="addressShort">
        {format_pub_key_shorter(address)}
        </span>
        </Tooltip>
        </div>
        <Tooltip placement="bottom" overlayStyle={{maxWidth: '500px'}} title={manager} arrowPointAtCenter>
        <div className="manager">Manager : { className === "fundPool" ? manager 
        : format_pub_key_shorter(manager)}</div>
        <div className="managerShort">Manager :{format_pub_key_shorter(manager)}</div>
        </Tooltip>
