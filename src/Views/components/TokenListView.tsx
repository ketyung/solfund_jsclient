...
export const TokenListView : React.FC <TokenListViewProps> = ({setSelected, setVisible}) => {
    ...
    <div className="tokenListView"> 
      ...
      style={{marginLeft:"10px",maxWidth:"90%",height:"40px",backgroundColor:"#334", color:"white"}} /></p>  
        ...
        <div className="logo">
        <Image width={30} title="Icon" alt="Icon" 
        style={{ verticalAlign: 'middle', marginTop:"5px", marginRight:"20px"}}
          height={30} preview={false}
          src={token.logoURI ? token.logoURI : "none"}
    ...
    useEffect(() => {
        ...
        provider.resolve().then((tokens) => {
            ...
            // filter for these only 
            const subListSymbs = ["SRM", "ETH", "BTC", "RAY", "mSOL", "USDC", "USDT"];

            const tokenList = tokens.filterByClusterSlug(cluster).getList();
    
            const subList = tokenList.filter((t)=>{

                return subListSymbs.includes(t.symbol);
            })

            const filteredList = keyword ? subList.filter ((t)=>{
